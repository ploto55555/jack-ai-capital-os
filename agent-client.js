async function callAgent(question, symbol, context) {
  try {
    ...
    return { answer: '连接 GPT Agent 失败。请检查 Vercel 部署。', usage: null };
  }
}
...
function improveChatReadability() {
  ...
  style.textContent = '.ai-chat-panel{min-height:430px!important}.chat-box{height:300px!important;max-height:300px!important;overflow-y:auto!important;padding:12px!important}.chat-box .msg{font-size:14px!important;line-height:1.65!important;white-space:pre-wrap!important}.chat-box .msg.ai{padding:14px!important;border-radius:10px!important}.ai-chat-form{grid-template-columns:1fr 58px 52px!important}.ai-chat-form input,.ai-chat-form button{height:42px!important}.usage-bubble{height:42px;border:1px solid #263442;background:#070b10;color:#8b98a8;border-radius:999px;display:flex;align-items:center;justify-content:center;font-size:11.5px;font-weight:800}.usage-bubble.thinking{color:#6cb6ff;border-color:#36506b}.usage-bubble.ok{color:#7ff0ad;border-color:#2f6848;background:#0d2418}.usage-bubble.warn{color:#f0b35a;border-color:#574018;background:#2c210f}.usage-bubble.err{color:#ff8a94;border-color:#5a1c25;background:#2b1014}.journal-list{max-height:150px!important;overflow-y:auto!important}';
...
function updateUsageBubble(usage, state) {
  ...
  if (!usage) { bubble.textContent = state === 'thinking' ? '◔ ...' : '◔ 0'; return; }
  ...
  bubble.title = '本次 ' + usage.totalTokens + ' tokens · 本月 $' + month.toFixed(2);
}
...
function enableAgentClient() {
  ...
  bubble.textContent = '◔ $' + getMonthlyCost().toFixed(2);
  ...
  form.addEventListener('submit', async function(event) {
    ...
    if (result.usage) updateUsageBubble(result.usage, 'ok'); else updateUsageBubble(null, answer.includes('失败') ? 'err' : 'ok');
